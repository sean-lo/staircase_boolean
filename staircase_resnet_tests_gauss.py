...
#            yield torch.randint(2,(1,self.n))*2-1, torch.rand(1)
            ...
            x = [random.gauss(0,1) for i in range(self.n)]
...
def output_losses_and_fourier_coeffs(eval_fn,n,iter_range,batch_size=1,track_fourier_coeffs_tuples=[],netparamsfolder=None):
    ...
    with torch.no_grad():
        ...
        #        print(inputs)
            ...
            #     print('Computing fourier coeffs')
            #     with torch.no_grad():
            #         tt = np.asarray([x for x in net(all_inputs)])
            #         fourier_coeffs = np.asarray(fwht(tt)) / 2**n
            #         running_coeffs.append([fourier_coeffs[j] for j in track_fourier_coeffs])
        ...
        # plt.plot(r_range[1:],running_losses[1:])
        # plt.show()

        # pickle.dump(running_coeffs,open('trained_wts/k20/coeffs.pkl','wb'))
        #
        # plot_fourier_coeffs(running_coeffs,'Learned stair Fourier coeffs')
...
def train_stair5_net():
    """
    Staircase, width of 50. n = 100, d = 5.
    """
    refresh_save_rate=200
    n = 100
    d = 5 # degree of staircase
    num_layers = 8
    layer_width = 50
    num_iter=100000
    learning_rate=0.01
    ...
    netparamsfolder = 'trained_wts/stair5_gauss/'
...
def train_parity5_net():
    ...
    netparamsfolder = 'trained_wts/parity5_gauss/'
    train_net_and_save_params(n,d,num_layers,layer_width,num_iter,learning_rate,batch_size,net_type,eval_fn,netparamsfolder,refresh_save_rate,erm,erm_num_samples)
